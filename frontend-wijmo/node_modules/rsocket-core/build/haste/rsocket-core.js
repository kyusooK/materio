...
var ExistingBufferModule = require('buffer');
...
const LiteBuffer = hasGlobalBuffer ? global.Buffer : Buffer$1;
 ...
 */
const toBuffer =
  typeof LiteBuffer.from === 'function'
    ? (...args) => {
        // Buffer.from(buffer) copies which we don't want here
        if (args[0] instanceof LiteBuffer) {
          return args[0];
        }
        return LiteBuffer.from.apply(LiteBuffer, args);
      }
    : (...args) => {
        // Buffer.from(buffer) copies which we don't want here
        if (args[0] instanceof LiteBuffer) {
        ...
        return new (LiteBuffer.bind.apply(LiteBuffer, [LiteBuffer, ...args]))();
      };
...
function encodeRoutes(...routes) {
  ...
  return LiteBuffer.concat(routes.map((route) => encodeRoute(route)));
}
...
function encodeRoute(route) {
  ...
  return LiteBuffer.concat([encodedLength, encodedRoute]);
}
...
function encodeWellKnownAuthMetadata(authType, authPayloadBuffer) {
  ...
  return LiteBuffer.concat([buffer, authPayloadBuffer]);
}
...
function encodeSimpleAuthMetadata(username, password) {
  ...
  return LiteBuffer.concat([buffer, usernameBuffer, passwordBuffer]);
}
...
function encodeBearerAuthMetadata(token) {
  ...
  return LiteBuffer.concat([buffer, tokenBuffer]);
}
