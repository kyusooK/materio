<template>
    <div style="max-height:80vh;">
        ...
        <div class="gs-bundle-of-buttons" style="max-height:10vh;">
            ...
            <v-btn @click="createCompanyDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Admin')">
                <v-icon small>mdi-minus-circle-outline</v-icon>회사 생성
            </v-btn>
            <v-dialog v-model="createCompanyDialog" width="500">
                <CreateCompanyCommand
                    @closeDialog="createCompanyDialog = false"
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['RowHeader','name','industry','foundedDate','photo',]" />
...
export default {
    ...
    methods:{
        createCompany(params){
            try{
                this.repository.invoke(this.getSelectedItem(), "createCompany", params)
                this.$EventBus.$emit('show-success','CreateCompany 성공적으로 처리되었습니다.')
            }catch(e){
